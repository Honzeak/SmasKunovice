using Mapsui;
using Mapsui.Fetcher;
using Mapsui.Layers;
using Moq;
using SmasKunovice.Avalonia.Models;

namespace SmasKunovice.Avalonia.Tests
{
    [TestFixture]
    public class DynamicPlanePositionProviderTests
    {
        private Mock<IDroneTagClient> _mockClient;
        
        [SetUp]
        public void Setup()
        {
            _mockClient = new Mock<IDroneTagClient>();
        }
        
        [Test]
        public void Constructor_RegistersForClientEvents()
        {
            // Act
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            
            // Assert
            _mockClient.VerifyAdd(c => c.MessageReceived += It.IsAny<IDroneTagClient.DronetagDataReceivedEventHandler>());
        }
        
        [Test]
        public async Task GetFeaturesAsync_WithNoMessages_ReturnsEmptyCollection()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            
            // Act
            var features = await provider.GetFeaturesAsync(new FetchInfo(new MSection(new MPoint(1,2).MRect, 1)));
            features = features.ToList();
            
            // Assert
            Assert.That(features, Is.Not.Null);
            Assert.That(features.Any(), Is.False);
        }
        
        [Test]
        public async Task GetFeaturesAsync_AfterMessageReceived_ReturnsFeatures()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            var messages = new List<DroneTagMessage>
            {
                new ("Drone1", -538000, -1185000, 500, 120, 45),
                new ("Drone2", -538100, -1185100, 550, 130, 50)
            };
            
            var eventArgs = new DroneTagMessageReceivedEventArgs
            {
                Messages = messages
            };
            
            // Simulate message received event
            _mockClient.Raise(c => c.MessageReceived += null, _mockClient.Object, eventArgs);
            
            // Act
            var features = await provider.GetFeaturesAsync(new FetchInfo(new MSection(new MPoint(1,2).MRect, 1)));
            var featuresList = features.ToList();
            
            // Assert
            Assert.That(featuresList, Is.Not.Null);
            Assert.That(featuresList, Has.Count.EqualTo(2));
            using (Assert.EnterMultipleScope())
            {
                // Verify that features match the messages
                Assert.That(featuresList[0], Is.InstanceOf<PointFeature>());
                Assert.That(featuresList[1], Is.InstanceOf<PointFeature>());
            }

            var drone1Feature = (PointFeature)featuresList[0];
            var drone2Feature = (PointFeature)featuresList[1];
            using (Assert.EnterMultipleScope())
            {
                Assert.That(drone1Feature["Message"], Is.Not.Null.And.InstanceOf<DroneTagMessage>());
                Assert.That(drone2Feature["Message"], Is.Not.Null.And.InstanceOf<DroneTagMessage>());
            }
            var message1 = (DroneTagMessage)drone1Feature["Message"]!;
            var message2 = (DroneTagMessage)drone2Feature["Message"]!;
            using (Assert.EnterMultipleScope())
            {
                Assert.That(message1.Id, Is.EqualTo("Drone1"));
                Assert.That(message1.Heading, Is.EqualTo(45));
                Assert.That(message2.Id, Is.EqualTo("Drone2"));
                Assert.That(message2.Heading, Is.EqualTo(50));
            }
        }
        
        [Test]
        public void DataHasChanged_TriggersDataChangedEvent()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            var eventWasRaised = false;
            DataChangedEventArgs capturedArgs = null;
            
            provider.DataChanged += (sender, args) =>
            {
                eventWasRaised = true;
                capturedArgs = args;
            };
            
            // Act
            provider.DataHasChanged();
            using (Assert.EnterMultipleScope())
            {
                Assert.That(eventWasRaised, Is.True);
                Assert.That(capturedArgs, Is.Not.Null);
            }
        }
        
        [Test]
        public void MessageReceived_TriggersDataChangedEvent()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            var eventWasRaised = false;
            
            provider.DataChanged += (sender, args) => eventWasRaised = true;
            
            var messages = new List<DroneTagMessage>
            {
                new DroneTagMessage("Drone1", -538000, -1185000, 500, 120, 45)
            };
            
            var eventArgs = new DroneTagMessageReceivedEventArgs
            {
                Messages = messages
            };
            
            // Act
            _mockClient.Raise(c => c.MessageReceived += null, _mockClient.Object, eventArgs);
            
            // Assert
            Assert.That(eventWasRaised, Is.True);
        }
        
        [Test]
        public void Dispose_DisposesClient()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            
            // Act
            provider.Dispose();
            
            // Assert
            _mockClient.Verify(c => c.Dispose(), Times.Once);
        }
        
        [Test]
        public async Task GetFeaturesAsync_UpdatesWithNewMessages()
        {
            // Arrange
            var provider = new DynamicPlanePositionProvider(_mockClient.Object);
            
            var firstMessages = new List<DroneTagMessage>
            {
                new DroneTagMessage("Drone1", -538000, -1185000, 500, 120, 45)
            };
            
            var firstEventArgs = new DroneTagMessageReceivedEventArgs
            {
                Messages = firstMessages
            };
            
            // Simulate first message
            _mockClient.Raise(c => c.MessageReceived += null, _mockClient.Object, firstEventArgs);
            
            // Act - First check
            var firstFeatures = (await provider.GetFeaturesAsync(new FetchInfo(new MSection(new MPoint(1,2).MRect, 1)))).ToList();
            
            // Now simulate a second message
            var secondMessages = new List<DroneTagMessage>
            {
                new ("Drone1", -538100, -1185100, 550, 130, 50),
                new ("Drone2", -538200, -1185200, 600, 140, 55)
            };
            
            var secondEventArgs = new DroneTagMessageReceivedEventArgs
            {
                Messages = secondMessages
            };
            
            _mockClient.Raise(c => c.MessageReceived += null, _mockClient.Object, secondEventArgs);
            
            // Act - Second check
            var secondFeatures = (await provider.GetFeaturesAsync(new FetchInfo(new MSection(new MPoint(1,2).MRect, 1)))).ToList();
            
            // Assert
            Assert.That(firstFeatures, Has.Count.EqualTo(1));
            Assert.That(secondFeatures, Has.Count.EqualTo(2));
            Assert.That(firstFeatures, Has.All.InstanceOf<PointFeature>());
            Assert.That(secondFeatures, Has.All.InstanceOf<PointFeature>());

            var droneTagMessage = firstFeatures[0]["Message"] as DroneTagMessage;
            Assert.That(droneTagMessage, Is.Not.Null);
            Assert.That(droneTagMessage.Id, Is.EqualTo("Drone1"));
            
            droneTagMessage = secondFeatures[0]["Message"] as DroneTagMessage;
            Assert.That(droneTagMessage, Is.Not.Null);
            Assert.That(droneTagMessage.Latitude, Is.EqualTo(-538100));
            Assert.That(droneTagMessage.Longitude, Is.EqualTo(-1185100));
        }
    }
}